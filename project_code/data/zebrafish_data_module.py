...
class ZebrafishDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage = None):
        ...
        fish_directories = os.listdir(data_dir)

        fish_dictionaries = []

        for fish_directory in fish_directories:
            # Get the right behaviour and metadata files.
            behaviour_path = fish_directory + '/*_behavior_log.hdf5'
            metadata_path = fish_directory + '/*_metadata.json'
            ...
            metadata_file = fish_directory + '/' + behaviour_ID \
                            + '_metadata.json'
            stimulus_file = fish_directory + '/' + behaviour_ID \
                            + '_stimulus_log.hdf5'
